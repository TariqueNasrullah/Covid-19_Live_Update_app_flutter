...
class _SecondPageState extends State<SecondPage> {
  ...
  static List<charts.Series<TimeSeriesData, DateTime>> _createDeathSampleData(
      List<DateDeathData> dDeathData) {
    List<TimeSeriesData> data = [];
            ...
            builder: (context, snapshot) {
              if (snapshot.hasData) {
                var pieData = [
                  ...
                  new PieDataClass(
                      "Recovered", snapshot.data.recovered, Color(0xff6bb5ff)),
                  new PieDataClass(
                      "Active Cases", snapshot.data.active, Colors.blueAccent),
                ];
                      ...
                      child: Container(
                          ...
                          child: Column(
                            children: <Widget>[
                              Expanded(
                                child: FutureBuilder<List<DateCaseData>>(
                                  ...
                                  builder: (context, lSnapshot) {
                                    if (lSnapshot.connectionState ==
                                            ConnectionState.done &&
                                        lSnapshot.hasData) {
                                      final timeSeriesDateCaseData = [];
                                      for (var r in lSnapshot.data) {
                                        timeSeriesDateCaseData.add(
                                            new TimeSeriesData(
                                                r.date, r.totalCase));
                                      }
                                      _seriesDateCaseData =
                                          _createSampleData(lSnapshot.data);
                              ...
                              Center(
                                child: Text('Date-Covid19 Case Graph'),
                              ),
                            ],
                          )),
                    ),
...
Future<List<DateCaseData>> fetchDateCaseDate() async {
  ...
  try {
    final response = await http
        .get("http://34.87.85.112:8080/dateCaseDataList/", headers: headers);
...
Future<List<DateDeathData>> fetchDateDeathData() async {
  ...
  try {
    final response = await http
        .get("http://34.87.85.112:8080/DateDeathDataList/", headers: headers);
